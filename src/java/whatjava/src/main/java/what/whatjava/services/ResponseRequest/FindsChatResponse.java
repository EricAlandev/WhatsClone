...
public class FindsChatResponse {
    ...
    public static List<ChatDTO> from(List<EntityChatTable> chats, EntityUser user){
        ...
        for(int i = 0; i < chats.size(); i++){
            ...
            System.out.println("SIZEE OF THE MESSAGFESSS " + otherUser.getMessages().size());

            if(otherUser != null && otherUser.getMessages().size() > 0){
                
                //last index;
                int lastIndex = actualChat.getMessageChat().size() - 1;

                lastMessage.setMessage(actualChat.getMessageChat().get(lastIndex).getMessageID().getMessage());

                lastMessage.setStatus(actualChat.getMessageChat().get(lastIndex).getMessageID().getStatus());

                lastMessage.setTime(TimeService.TextConvert(actualChat.getMessageChat().get(lastIndex).getMessageID().getTime())); 
